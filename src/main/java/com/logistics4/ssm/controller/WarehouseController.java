...
public class WarehouseController {
    ...
    public String addWareHouse(@RequestParam(value = "area") String area,@RequestParam(value = "name") String name,
                               @RequestParam(value = "size") int size,@RequestParam(value = "linkman") String linkman,
                               @RequestParam(value = "phonenumber") String phonenumber,@RequestParam("wareHouseType") String wareHouseType){
        int i = warehouseService.addWarehouse(area, name,wareHouseType, size, linkman, phonenumber);
        if(i<=0){
            return "fail";
        }
        return "homepage";
